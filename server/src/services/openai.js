...
/**
 * Calls the OpenAI chat completions API and parses the moderation verdict.
...
export async function moderateArticle({ title, content, tags = [] }) {
  ...
  try {
    ...
    logger.warn({ raw }, 'OpenAI returned non-JSON content');
    throw new Error(`OpenAI JSON parse failed: ${err.message}`);
  }

  return normalize(parsed, env.OPENAI_MODERATION_MODEL, json);
}
